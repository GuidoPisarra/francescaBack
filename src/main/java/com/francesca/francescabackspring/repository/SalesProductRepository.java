...
public interface SalesProductRepository extends JpaRepository<SalesProduct, Integer> {
    @Query("SELECT new com.francesca.francescabackspring.DTO.ReportSalesDay(s.id, sp.idProduct, sp.idSale, p.description, sp.quantity, (sp.quantity * sp.price),sp.SaleProductDate, sp.typePayment )" +
            ...
            "WHERE sp.active = 0 "+
            "ORDER BY sp.SaleProductDate DESC"
    )
